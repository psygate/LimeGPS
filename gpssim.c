...
void* gps_task(void* arg)
{
	...
	int stored = 0;
	double velocity_storage = 0;
	double DELTA_VELOCITY = 0.4;
	double DELTA_ANGULAR = 15;
	...
	for (iumd = 1; iumd < numd; iumd++)
	{
		...
		if (igrx % 300 == 0) // Every 30 seconds
		{
			...
			if (verb)
			{
				//printf("\n");
				gps2date(&grx, &t0);
				snprintf(scratchpad, SCRATCHPAD_SIZE, "%4d/%02d/%02d,%02d:%02d:%02.0f (%d:%.0f)\n", t0.y, t0.m, t0.d, t0.hh, t0.mm, t0.sec, grx.week, grx.sec);
				waddstr(s->meta.scr, scratchpad);
				snprintf(scratchpad, SCRATCHPAD_SIZE, "xyz = %11.1f, %11.1f, %11.1f\n", xyz[iumd][0], xyz[iumd][1], xyz[iumd][2]);
				waddstr(s->meta.scr, scratchpad);

				xyz2llh(xyz[iumd], llh);
				snprintf(scratchpad, SCRATCHPAD_SIZE, "llh = %11.6f, %11.6f, %11.1f\n", llh[0] * R2D, llh[1] * R2D, llh[2]);
				waddstr(s->meta.scr, scratchpad);
		...
		// Update time counter
		snprintf(scratchpad, SCRATCHPAD_SIZE, "Time into run = %4.1f\n", subGpsTime(grx, g0));
		mvwaddstr(s->meta.scr, 14, 0, scratchpad);
